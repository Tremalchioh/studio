        ...
        <CardFooter>
          <AlertDialog open={isQuizDialogOpen} onOpenChange={setIsQuizDialogOpen}>
            <AlertDialogTrigger asChild>
              <Button 
                size="lg" 
                className="w-full text-accent-foreground rounded-lg text-base" // Removed bg-accent hover:bg-accent/90
              ...
              >
                {quizData.options.map((option: QuizOption) => {
                  ...
                  return (
                    <Label
                      key={option.id}
                      htmlFor={`quiz-option-${option.id}-${subLesson.id}`} 
                      className={cn(
                    ...
                    >
                      <RadioGroupItem
                        value={option.id}
                        id={`quiz-option-${option.id}-${subLesson.id}`} 
                        disabled={quizSubmitted}
                ...
                {!quizSubmitted ? (
                  <>
                    ...
                    <Button
                      ...
                      // className="bg-primary hover:bg-primary/90" // hover is handled by default variant now
                    >
                      Отправить ответ
                    </Button>
                  </>
                ) : (
                  <Button onClick={handleQuizDialogClose} className="w-full">Закрыть</Button> // hover is handled by default variant now
                )}
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
